...
```bash
shellgeiai solve "CSVの3列目の合計を出してください" --runner docker --mode parallel --parallelism 3
```
...
- Planner と Selector は現時点では単純な既定実装ですが、複数 worker 前提の contract と最小 progress 出力は導入済みです
- `DockerRunner` は最小実装段階で、CLI から選べますが実機 Docker を使った統合テストや image 運用はまだ未整備です
