...
- 実際の solve は複数 worker の並列実行に対応済み
- `src/solve/session/solveSession.js` と `src/solve/orchestration/orchestrator.js` が現行フローの中心
- `src/solve/planning/planner.js` は LLM planner の結果を worker task へ正規化する
- worker retry loop は `src/solve/worker/executeWorkerTask.js`、1 attempt の実行は `src/solve/worker/attemptRunner.js`
- attempt / candidate object の構築は `src/solve/worker/attemptFactory.js`、worker 停止理由と deadline 判定は `src/solve/worker/stopReason.js`
- orchestration 側の停止制御は `src/solve/orchestration/executionControl.js`、実行サマリ整形は `src/solve/orchestration/executionSummary.js`
- `src/solve/selection/selector.js` は最小実装
- `src/execution/runner/localRunner.js` と `src/execution/runner/dockerRunner.js` が実行基盤
- CLI 引数解析は `src/cli/options/` へ分割済みで、`src/cliOptions.js` は互換用 re-export として残っている
- `src/io/problem/`、`src/execution/safety/`、`src/io/logs/` は将来拡張に備えて分離済み

つまり、現行実装は Docker runner と複数 worker の並列実行を取り込みつつ、SubAgent 戦略や判定・選択の拡張余地を残している段階です。
...
- 各 worker に strategy 名だけでなく、役割説明と retry 方針を渡す
...
- コンテナ制限値
- 採用されなかった候補も含む結果保存
- 解析や corpus 化に使えるメタデータ保存

並列実行では、成功した 1 本だけでなく「他に何が走っていたか」が重要な分析資産になります。

## 段階的移行ポリシー

- まずは責務境界を固め、複数 worker の result shape を崩さずに機能を拡張する
- `Runner` は先に limits と sandbox policy を表現できる interface に広げる
- Docker 実装は、CLI 契約とログ構造を保ちながら制限値と再現性を強化する
